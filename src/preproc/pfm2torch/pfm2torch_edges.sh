...
for i in /media/simonovm/Slow/datasets/Locate/match_descriptors_dataset/**/*.pfm; do # Whitespace-safe and recursive
	matlab -nodisplay -nosplash -nodesktop -r "data=single(parsePfm('$i')); data=significantEdges(data, 4); imwrite(data, ['$i' '_edges.png']); save('/home/simonovm/tmp/tmp.mat', 'data'); exit"
