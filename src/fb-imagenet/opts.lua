...
function M.parse(arg)
	...
	cmd:text('Options:')
	cmd:option('-runName', '', '')  --!!!!!!
	cmd:option('-save', os.getenv('HOME')..'/workspace/E/locate', 'subdirectory to save/log experiments in')
	...
	cmd:option('-datasetPostfix', '', 'dataset folder postfix')
    ...
    cmd:option('-inputMode', 'allshadesG', 'allshades | allshadesG | camnorm | depth | shadesnormG')
    cmd:option('-patchSize', 64, 'w or h of a patch')   
    cmd:option('-patchSampleMinScaleF', 0.5, 'base patch sampling scale range [min]')
    cmd:option('-patchSampleMaxScaleF', 2, 'base patch sampling scale range [max]')
    cmd:option('-patchSampleRotMaxPercA', 1/18, 'base patch sampling rotation range (0 = disabled ..up to 1 == full angle [-pi,pi])')    
    cmd:option('-patchJitterRotMaxPercA', 1/90, 'extra jittering for pos patches: whether should rotate patches by random angle (0 = disabled ..up to 1 == full angle [-pi,pi])')
    cmd:option('-patchJitterMaxScaleF', 1.1, 'extra jittering for pos patches: max factor for randomly up/down-scaling patches. 1 == disabled.')
    cmd:option('-patchSampleWarpF', 0, 'max displacement of nodes when warping samples (factor of patchsize), 0.1 seems ok. 0 == disabled.')
    cmd:option('-patchSampleClrJitter', 0, 'max additive value to hsv of training samples, 0.1 seems ok. 0 == disabled.')
    cmd:option('-patchSampleNegThres', 0.7, 'patchSampleNegDist threshold for negatives (>0: maximum, <0: minimum)')
    cmd:option('-patchSampleNegDist', 'inter', 'distance type to distinguish neg samples inter | center')    
    cmd:option('-sampleWeightMode', '', 'weighting of training samples \'\' | synthstd | dedges')    
    cmd:option('-sampleWeightFactor', 0.1, 'normalization factor weighting of training samples; for dedges ~ 0.1?')
	--
    ...
    cmd:option('-cudnnMode', 'default', 'fastest|benchmark|default')
